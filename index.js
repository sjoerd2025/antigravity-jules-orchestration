...
class SessionQueue {
  ...
  markProcessing(id) { const item = this.queue.find(i => i.id === id); if (item) { item.status = 'processing'; item.startedAt = new Date().toISOString(); } }
  ...
  list() { return this.queue.map(i => ({ id: i.id, title: i.config.title || 'Untitled', priority: i.priority, status: i.status, addedAt: i.addedAt, sessionId: i.sessionId, startedAt: i.startedAt, completedAt: i.completedAt, failedAt: i.failedAt, error: i.error })); }
