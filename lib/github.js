 ...
 * Fetch issues, PRs, and repository information for Gemini session context
 */
 ...
 */
function githubRequest(path, token = null, { method = 'GET', body = null } = {}) {
  return new Promise((resolve, reject) => {
    const options = {
      ...
      method,
      headers: {
        'User-Agent': 'Gemini-MCP-Server/3.0.0',
...
/**
 * Format issue context for Gemini prompt
 */
...
// ─── Label Helpers (used by TaskQueue) ──────────────────────

/**
 * Add a label to an issue.
 */
export async function addLabelToIssue(owner, repo, issueNumber, label, token = null) {
 ...
 */
export async function removeLabelFromIssue(owner, repo, issueNumber, label, token = null) {
  return await githubRequest(
 ...
 */
export async function getIssueComments(owner, repo, issueNumber, token = null) {
  return await githubRequest(
