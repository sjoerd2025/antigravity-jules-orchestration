 ...
 * Execute multiple Gemini sessions in parallel with progress tracking
 */
...
export class BatchProcessor {
    constructor(geminiRequest, createSession) {
    ...
    async getBatchStatus(batchId) {
        ...
        const statusPromises = batch.sessions.map(async (session) => {
            try {
                const details = await this.geminiRequest('getSession', { sessionId: session.id });
                return {
                    id: session.id,
                    title: session.title,
                    state: details.state,
    ...
    async approveAllInBatch(batchId) {
        ...
        const approvalResults = await Promise.all(
            waitingSessions.map(async (session) => {
                try {
                    await this.geminiRequest('approvePlan', { sessionId: session.id });
