 ...
 * Real-time tracking and statistics for Gemini sessions
 */
...
export class SessionMonitor {
    ...
    async getAllSessions(forceRefresh = false) {
        ...
        try {
            const result = await this.geminiRequest('listSessions', {});
    ...
    async getSessionTimeline(sessionId) {
        try {
            const [session, activities] = await Promise.all([
                this.geminiRequest('getSession', { sessionId }),
                this.geminiRequest('getSession', { sessionId })
            ]);
    ...
    async monitorAll() {
        ...
        const detailed = await Promise.all(
            active.slice(0, 20).map(async (session) => {
                ...
                try {
                    const details = await this.geminiRequest('getSession', { sessionId: id });
                    return {
                        id,
                        title: details.title,
                        state: details.state,
